...
class RECActorCritic(nnx.Module):
    ...
    def __call__(self, obs):
        ...
        alpha = nnx.softplus(logit).squeeze(axis=-1)
...
def make_train(config, env):
    ...
    optimizer_batteries = nnx.Optimizer(network_batteries, tx)
...
def train(env: RECEnv, config, train_state, rng, validate=True, freq_val=None, val_env=None, val_params=None, val_rng=None, val_num_iters=None):
    ...
    def _update_step(runner_state_plus, curr_iter):
        ...
        def _env_step(runner_state, unused):
            ...
            # pi, value_rec = network_rec(network_rec.prepare_data(rec_obsv))
        ...
        def _update_epoch(update_state, unused):
            def _update_minbatch(train_state, batch_info):
                ...
                # print('fghjkhvcxzdfxhcgv')
                # print(jax.tree.map(lambda x: x.shape, batch_info))
                advantages_batteries, advantages_rec = advantages
                targets_batteries, targets_rec = targets

                # def _loss_fn(networks, traj_batch, gae, targets):
                #     networks_batteries, network_rec = networks
                #     gae_batteries, gae_rec = gae
                #     targets_batteries, targets_rec = targets
                #
                #     # RERUN NETWORK
                #     pi_batteries, values_batteries = networks_batteries(traj_batch.obs_batteries)
                #     log_prob_batteries = pi_batteries.log_prob(traj_batch.actions_batteries)
                #
                #     pi_rec, values_rec = network_rec(traj_batch.obs_rec)
                #     log_probs_rec = pi_rec.log_prob(traj_batch.actions_rec)
                #
                #     # CALCULATE VALUE LOSS
                #     value_pred_clipped = traj_batch.values_batteries + (
                #             values_batteries - traj_batch.values_batteries
                #     ).clip(-config["CLIP_EPS"], config["CLIP_EPS"])
                #     value_losses = jnp.square(values_batteries - targets_batteries)
                #     value_losses_clipped = jnp.square(value_pred_clipped - targets_batteries)
                #     value_loss_batteries = (
                #         0.5 * jnp.maximum(value_losses, value_losses_clipped).mean()
                #     )
                #
                #     value_pred_clipped = traj_batch.value_rec + (
                #             values_rec - traj_batch.value_rec
                #     ).clip(-config["CLIP_EPS"], config["CLIP_EPS"])
                #     value_losses = jnp.square(values_batteries - targets_rec)
                #     value_losses_clipped = jnp.square(value_pred_clipped - targets_rec)
                #     value_loss_rec = (
                #             0.5 * jnp.maximum(value_losses, value_losses_clipped).mean()
                #     )
                #
                #     # CALCULATE ACTOR LOSS
                #     ratio = jnp.exp(log_prob_batteries - traj_batch.log_prob)
