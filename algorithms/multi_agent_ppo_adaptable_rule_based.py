...
def make_train(config, env:RECEnv):
    ...
    schedule_rec = schedule_builder(config['LR_REC'], config['LR_REC_MIN'], config['FRACTION_DYNAMIC_LR_REC'], config['NUM_MINIBATCHES_REC'])
    ...
    graph_def, state = nnx.split((network_rec, optimizer_rec))
...
class RunnerState(NamedTuple):
    ...
    is_battery_policy_only_market: jnp.array
...
def train_wrapper(env:RECEnv, config, network_rec, optimizer_rec,
    ...
    def update_val_info(val_info, train_state, i):
        ...
        network_rec, _ = nnx.merge(train_state.graph_def, train_state.state)

        val_info = jax.device_put(val_info, device=jax.devices('cpu')[0])
        jax.tree.map(update, val_infos, val_info)

        utils.save_state_multiagent(directory, None, network_rec, config, world_metadata, is_checkpoint=True, num_steps=i)

    @partial(nnx.jit, static_argnums=(0, 1, 7, 8, 9, 11))
    def train(env: RECEnv, config, is_agent_adaptable, initial_policies, network_rec, optimizer_rec, rng, validate=True,
              freq_val=None, val_env=None, val_rng=None, val_num_iters=None):
        ...
        def _update_step(runner_state, curr_iter):
            ...
            runner_state, traj_batch, last_val_rec = collect_trajectories(runner_state, config, env, rec_rule_based_policy, is_agent_adaptable)

            advantages_rec, targets_rec = _calculate_gae(traj_batch, last_val_rec, config)
            ...
            if validate:
                _ = jax.lax.cond(curr_iter % freq_val == 0,
                                 lambda: io_callback(update_val_info,
                                                     None,
                                                     test_networks(val_env, TrainState(*nnx.split((runner_state.network_rec, runner_state.optimizer_rec))), is_agent_adaptable, initial_policies,
                                                                   val_num_iters, config, val_rng, rec_rule_based_policy=rec_rule_based_policy,
                                                                   curr_iter=curr_iter, print_data=True),
                                                     TrainState(*nnx.split((runner_state.network_rec, runner_state.optimizer_rec))),
                                                     curr_iter // freq_val,
                                                     ordered=True),
                                 lambda: None)
        ...
        is_battery_policy_only_market = jnp.tile(jnp.array(initial_policies), config['NUM_ENVS']).reshape((config['NUM_ENVS'], -1))

        runner_state = RunnerState(network_rec=network_rec,
                                   ...
                                   is_battery_policy_only_market=is_battery_policy_only_market)
    ...
    is_agent_adaptable = jnp.array(config['IS_AGENT_ADAPTABLE'])
    initial_policies = jnp.array(config['INITIAL_POLICIES'])

    runner_state = train(env, config, is_agent_adaptable, initial_policies, network_rec, optimizer_rec, rng, validate, freq_val, val_env, val_rng, val_num_iters)

    print('Saving...')

    t0 = time()

    utils.save_state_multiagent(directory, None, runner_state.network_rec, config, world_metadata, infos, val_infos, is_checkpoint=False)
...
def collect_trajectories(runner_state: RunnerState, config, env, rec_rule_based_policy, is_agent_adaptable):
    ...
    def _env_step(runner_state: RunnerState):
        ...
        actions_batteries_only_market = jnp.zeros((config['NUM_ENVS'], config['NUM_BATTERY_AGENTS']))
        demand = runner_state.last_obs_batteries[..., env._obs_battery_agents_idx['demand']]
        generation = runner_state.last_obs_batteries[..., env._obs_battery_agents_idx['generation']]

        actions_batteries_battery_first = (generation - demand) / runner_state.env_state.battery_states.electrical_state.v

        print('bf', actions_batteries_battery_first.shape)
        print('om', actions_batteries_only_market.shape)

        actions_batteries = jnp.where(runner_state.is_battery_policy_only_market, actions_batteries_only_market, actions_batteries_battery_first)

        print('ab', actions_batteries.shape)
        ...
        actions_second = {agent: jnp.zeros((config['NUM_ENVS'],)) for agent in env.battery_agents}
        ...
        info['is_battery_policy_only_market'] = runner_state.is_battery_policy_only_market
        ...
        transition = Transition(
            ...
            info,
            runner_state.done_prev_rec,
            runner_state.last_lstm_state_rec
        )

        new_done_prev_rec = done_rec if config['NETWORK_TYPE_REC'] == 'recurrent_actor_critic' else runner_state.done_prev_rec

        is_battery_policy_only_market = jnp.where(jnp.logical_and(is_agent_adaptable[None, :], info['weig_reward']['r_glob'] < info['weig_reward']['r_glob'].mean(axis=1, keepdims=True)),
                                                  jnp.logical_not(runner_state.is_battery_policy_only_market),
                                                  runner_state.is_battery_policy_only_market)
    ...
    return runner_state, traj_batch, last_val_rec


def _calculate_gae(traj_batch, last_val_rec, config):
    ...
    return advantages_rec, advantages_rec + traj_batch.value_rec
...
def test_networks(env:RECEnv, train_state:TrainState, is_agent_adaptable, initial_policies, num_iter, config, rng, rec_rule_based_policy, curr_iter=0, print_data=False):
    ...
    def _env_step(runner_state, unused):
        obsv_batteries, env_state, is_battery_policy_only_market, act_state_rec, rng, next_profile_index = runner_state

        # print('aaaaa', obsv_batteries[:config['NUM_RL_AGENTS']].shape)

        actions_batteries_only_market = jnp.zeros((config['NUM_BATTERY_AGENTS'],))
        demand = obsv_batteries[..., env._obs_battery_agents_idx['demand']]
        generation = obsv_batteries[..., env._obs_battery_agents_idx['generation']]

        actions_batteries_battery_first = (generation - demand) / env_state.battery_states.electrical_state.v

        actions_batteries = jnp.where(is_battery_policy_only_market,
                                      actions_batteries_only_market, actions_batteries_battery_first)
        ...
        info['is_battery_policy_only_market'] = is_battery_policy_only_market
        ...
        is_battery_policy_only_market = jnp.where(jnp.logical_and(is_agent_adaptable,
                                                                  info['weig_reward']['r_glob'] < info['weig_reward'][
                                                                      'r_glob'].mean()),
                                                  jnp.logical_not(is_battery_policy_only_market),
                                                  is_battery_policy_only_market)

        runner_state = (obs_batteries, env_state, is_battery_policy_only_market, act_state_rec, rng, next_profile_index)
        return runner_state, info

    obsv_batteries = jnp.vstack([obsv[a] for a in env.battery_agents])

    runner_state = (obsv_batteries, env_state, jnp.array(initial_policies), act_state_rec, rng, 1)
