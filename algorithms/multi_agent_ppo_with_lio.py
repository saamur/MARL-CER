...
from algorithms.networks_lio import StackedIncentiveNetwork
...
def make_train(config, env:RECEnv):
    ...
    def get_optim(name, scheduler):
        if name == 'adam':
            return optax.adam(learning_rate=scheduler, eps=0., eps_root=1e-10)
        elif name == 'adamw':
            print('ADAMW')
            return optax.adamw(learning_rate=scheduler, eps=0., eps_root=1e-10)
        elif name == 'sgd':
    ...
    if config['USE_WEIGHT_DECAY']:
        tx_pol = optax.chain(
            ...
            get_optim(config['OPTIMIZER_POLICY'], schedule_policy)
        )
        tx_inc = optax.adamw(learning_rate=schedule_incentives, eps=1e-5)
    else:
        tx_pol = optax.chain(
            optax.clip_by_global_norm(config['MAX_GRAD_NORM']),
            get_optim(config['OPTIMIZER_INCENTIVES'], schedule_incentives)
        )
        tx_inc = optax.adam(learning_rate=schedule_incentives, eps=1e-5)
...
class RunnerState(NamedTuple):
    ...
    networks_incentives: Union[RECActorCritic, RECRecurrentActorCritic]
...
def collect_trajectories(runner_state: RunnerState, config, env):
    ...
    def _env_step(runner_state: RunnerState):
        ...
        if config['NUM_BATTERY_FIRST_AGENTS'] > 0:
            ...
            print(actions_battery_first.shape)
        ...
        if config['NUM_ONLY_MARKET_AGENTS'] > 0:
            ...
            print(actions_only_market.shape)
        ...
        obsv, env_state, reward_env, done, info = env.step(rng_step, env_state, actions_dict) #, stop_gradient_r_op=True, stop_gradient_r_deg=True, stop_gradient_r_clip=True, stop_gradient_r_trad=True)
        ...
        int_rewards_separate_num_batteries_first = network_incentives(last_obs_rl_num_batteries_first, others_actions[:config['NUM_RL_AGENTS']])
        int_rewards_separate = jnp.swapaxes(int_rewards_separate_num_batteries_first, 0, 1)
        int_rewards_mat = add_zero_diag(int_rewards_separate)
...
def _calculate_gae(ext_reward, int_reward, dones, values, last_val, config):
    ...
    tot_rewards = ext_reward[..., :config['NUM_RL_AGENTS']] + int_reward[..., :config['NUM_RL_AGENTS']]
    ...
    _, advantages = jax.lax.scan(
        ...
        (dones[..., :config['NUM_RL_AGENTS']], values[..., :config['NUM_RL_AGENTS']], tot_rewards),
        reverse=True,
        unroll=32,
    )
    targets = advantages + values[..., :config['NUM_RL_AGENTS']]
...
def update_incentive_network(runner_state:RunnerState, env, config):
    ...
    def inc_loss(net_inc, runner_state:RunnerState):
        ...
        def loss_first_term(traj_batch_second:Transition):
            ...
            _, returns = jax.lax.scan(compute_return,
                                      ...
                                      traj_batch_second.ext_rewards[..., :config['NUM_RL_AGENTS']],
                                      reverse=True,
                                      unroll=32)
        ...
        def loss_second_term(int_rewards_mat):
            # others_actions = get_others_actions(jnp.squeeze(traj_batch_first.actions, axis=-1))
            #
            # last_obs_rl_num_batteries_first = jax.tree.map(
            #     lambda x: jnp.moveaxis(x, (2, 0, 1), (0, 1, 2))[:config['NUM_RL_AGENTS']],
            #     traj_batch_first.obs)
            #
            # int_rewards_separate_num_batteries_first = runner_state.networks_incentives(last_obs_rl_num_batteries_first,
            #                                                others_actions[:config['NUM_RL_AGENTS']])
            # int_rewards_separate = jnp.moveaxis(int_rewards_separate_num_batteries_first, (0, 1, 2), (2, 0, 1))
            # int_rewards_mat = add_zero_diag(int_rewards_separate)
            gammas = config['GAMMA'] ** jnp.arange(config['NUM_STEPS'])
            norms = jnp.abs(int_rewards_mat).sum(axis=-1)

            loss = gammas[:, None, None] * norms

            loss = loss.mean(axis=1)

            print('ffffffkdl', loss.shape)
        ...
        #fixme debug
        # traj_batch_first = jax.lax.stop_gradient(traj_batch_first)
        # last_val_batteries = jax.lax.stop_gradient(last_val_batteries)
        ...
        # runner_state._replace(networks_incentives=net_inc)
        # runner_state, traj_batch_first, last_val_batteries = collect_trajectories(runner_state, config, env)

        advantages, targets = _calculate_gae(traj_batch_first.ext_rewards,
                                             int_rewards,
                                             traj_batch_first.done,
                                             traj_batch_first.values,
                                             last_val_batteries,
                                             config)

        # advantages, targets = jax.lax.stop_gradient((advantages, targets))
        ...
        # fixme debug
        # traj_batch_second = jax.lax.stop_gradient(traj_batch_second)
        # last_val_batteries = jax.lax.stop_gradient(last_val_batteries)

        loss_first = loss_first_term(traj_batch_second)
        loss_second = loss_second_term(int_rewards_mat)
...
def test_networks(env: RECEnv, train_state: TrainState, num_iter, config, rng, rec_rule_based_policy, curr_iter=0,
    ...
    def _env_step(runner_state, unused):
        ...
        others_actions = get_others_actions(actions)

        int_rewards_separate = network_incentives(obsv_rl, others_actions[:config['NUM_RL_AGENTS']])
        int_rewards_mat = add_zero_diag(int_rewards_separate)
        int_rewards = int_rewards_mat.sum(axis=-2)
