...
def make_train(config, env:RECEnv, network_batteries=None):
    ...
    config['BATTERY_OBSERVATION_SPACE_SIZE_ACT'] = env.observation_space(env.battery_agents[0]).shape[0]
    config['BATTERY_OBSERVATION_SPACE_SIZE_ONLY_CRI'] = 1
...
def collect_trajectories(runner_state: RunnerState, config, env, for_batteries_update):
    ...
    def _env_step(runner_state: RunnerState):
        ...
        last_obs_batteries_only_cri_num_batteries_first = jnp.swapaxes(runner_state.rec_cri_separate, 0, 1)
        ...
        if config['NETWORK_TYPE_BATTERIES'] == 'recurrent_actor_critic':
            ...
            pi, value_batteries = network_batteries(last_obs_batteries_num_batteries_first[:config['NUM_RL_AGENTS']], last_obs_batteries_only_cri_num_batteries_first[:config['NUM_RL_AGENTS']])
    ...
    def _env_step(runner_state, unused):
        ...
        if config['NUM_RL_AGENTS'] > 0:
            ...
            if config['NETWORK_TYPE_BATTERIES'] == 'recurrent_actor_critic':
                ...
                pi, _ = networks_batteries(obsv_batteries[:config['NUM_RL_AGENTS']], None, return_cri=False)
        ...
        if not config['USE_REC_RULE_BASED_POLICY']:
            if config['NETWORK_TYPE_REC'] == 'recurrent_actor_critic':
                ...
                pi, _ = network_rec(rec_obsv, return_cri=False)
            actions_rec = pi.mean()
        else:
