...
from algorithms.networks import StackedActorCritic, StackedRecurrentActorCritic, StackedAsymmetricActorCritic, RECActorCritic, RECRecurrentActorCritic, AsymmetricRECActorCritic
...
def make_train(config, env:RECEnv, network_batteries=None):
    ...
    print('PPO FICTITIOUS WITH VAL IN OBS')
    ...
    if 'VALUES_IN_OBS_SPACE' not in config.keys():
        config['VALUES_IN_OBS_SPACE'] = False
    ...
    config['BATTERY_OBSERVATION_SPACE_SIZE_ACT'] = env.observation_space(env.battery_agents[0]).shape[0] + (1 if config.get('VALUES_IN_OBS_SPACE', False) else 0)
    config['BATTERY_OBSERVATION_SPACE_SIZE_ONLY_CRI'] = 0 if config['VALUES_IN_OBS_SPACE'] else 1
    ...
    config['REC_OBS_KEYS_ACT'] = tuple(env._obs_rec_keys)
    config['REC_OBS_KEYS_ACT'] += ('values_batteries',) if config['VALUES_IN_OBS_SPACE'] else ()
    config['REC_OBS_KEYS_CRI'] = tuple(env._obs_rec_keys + ['values_batteries'])
    config['NUM_BATTERY_AGENTS'] = env.num_battery_agents
    config['PASSIVE_HOUSES'] = (env.num_passive_houses>0)
    config['REC_OBS_IS_SEQUENCE'] = {**env.obs_is_sequence_rec, 'values_batteries': True}
    config['REC_OBS_IS_LOCAL'] = {**env.obs_is_local_rec, 'values_batteries': True}
    config['REC_OBS_IS_NORMALIZABLE'] = {**env.obs_is_normalizable_rec, 'values_batteries': True}
    ...
    if network_batteries is None:
        _rng = nnx.Rngs(123)
        network_batteries = utils.construct_battery_net_from_config_multi_agent(config, _rng, obs_space_size=config['BATTERY_OBSERVATION_SPACE_SIZE_ACT'], num_nets=config['NUM_RL_AGENTS'])
    _rng = nnx.Rngs(222)
...
class RunnerState(NamedTuple):
    ...
    network_batteries: StackedAsymmetricActorCritic
    network_rec: AsymmetricRECActorCritic
...
class Transition(NamedTuple):
    ...
    obs_batteries_only_cri: jnp.ndarray  #
    obs_rec: jnp.ndarray
...
def train_wrapper(env:RECEnv, config, network_batteries, optimizer_batteries, network_rec, optimizer_rec,
    ...
    def end_update_step(info, i):
        if len(infos) == 0:
            ...
            infos.update(jax.tree.map(lambda x: np.empty_like(x, shape=(config['NUM_UPDATES']*2,) + x.shape[1:]), info))
    ...
    def train(env: RECEnv, config, network_batteries, optimizer_batteries, network_rec, optimizer_rec, rng, validate=True, freq_val=None, val_env=None,
        ...
        def _update_step(runner_state, curr_iter):
            ...
            metric1 = traj_batch.info

            # runner_state = nnx.cond(update_batteries,
            #                         lambda run, traj, adv_bat, tar_bat, adv_rec, tar_rec, con : update_batteries_network(run, traj, adv_bat, tar_bat, con)[0],
            #                         lambda run, traj, adv_bat, tar_bat, adv_rec, tar_rec, con : update_rec_network(run, traj, adv_rec, tar_rec, con)[0],
            #                         runner_state, traj_batch, advantages_batteries, targets_batteries, advantages_rec, targets_rec, config)
            ...
            metric2 = traj_batch.info

            metric = jax.tree.map(lambda x, y: jnp.stack((x, y), axis=0), metric1, metric2)
        ...
        runner_state = RunnerState(network_batteries=network_batteries,
                                   ...
                                   rec_cri_separate=jnp.zeros((config['NUM_ENVS'], config['NUM_BATTERY_AGENTS'], 1)),
...
def collect_trajectories(runner_state: RunnerState, config, env, for_batteries_update):
    ...
    def _env_step(runner_state: RunnerState):
        ...
        last_obs_batteries_num_batteries_first = jnp.swapaxes(runner_state.last_obs_batteries, 0, 1)[:config['NUM_RL_AGENTS']]
        rec_cri_separate_num_batteries_first = jnp.swapaxes(runner_state.rec_cri_separate, 0, 1)[:config['NUM_RL_AGENTS']]
        if config['VALUES_IN_OBS_SPACE']:
            last_obs_batteries_num_batteries_first = jnp.concat((last_obs_batteries_num_batteries_first,rec_cri_separate_num_batteries_first), axis=-1)
            last_obs_batteries_only_cri_num_batteries_first = None
        else:
            last_obs_batteries_only_cri_num_batteries_first = rec_cri_separate_num_batteries_first
        ...
        if config['NETWORK_TYPE_BATTERIES'] == 'recurrent_actor_critic':
            ...
            pi, value_batteries, lstm_act_state, lstm_cri_state = network_batteries(last_obs_batteries_num_batteries_first, prev_act_state_num_batteries_first, prev_cri_state_num_batteries_first)
            lstm_act_state_batteries, lstm_cri_state_batteries = jax.tree.map(lambda x : jnp.swapaxes(x, 0, 1), (lstm_act_state, lstm_cri_state))
        else:
            pi, value_batteries = network_batteries(last_obs_batteries_num_batteries_first, last_obs_batteries_only_cri_num_batteries_first)
        ...
        transition = Transition(
            ...
            runner_state.last_obs_batteries, runner_state.rec_cri_separate, rec_obsv,
            info,
    ...
    import jax.numpy as my_jnp
    import numpy as my_np
    import jax as my_jax

    nans = jax.tree.map(lambda x: jnp.isnan(x).any(), traj_batch)

    # jax.lax.cond(nans.actions_batteries, lambda : jax.debug.breakpoint(), lambda : None)

    # jax.debug.print('{x}', x=traj_batch)
        ...
        def _update_minbatch(net_and_optim, traj_batch, advantages, targets):
            ...
            def _loss_fn_batteries(network, traj_batch, gae, targets):
                traj_batch_data = (traj_batch.obs_batteries,
                                   traj_batch.obs_batteries_only_cri,
                                   traj_batch.actions_batteries,
                                   traj_batch.values_batteries,
                                   traj_batch.log_prob_batteries)
                traj_batch_obs, traj_batch_obs_only_cri, traj_batch_actions, traj_batch_values, traj_batch_log_probs = jax.tree.map(lambda x: x.swapaxes(0, 1), traj_batch_data)
                traj_batch_obs, traj_batch_obs_only_cri, traj_batch_actions = traj_batch_obs[:config['NUM_RL_AGENTS']], traj_batch_obs_only_cri[:config['NUM_RL_AGENTS']], traj_batch_actions[:config['NUM_RL_AGENTS']]

                if config['VALUES_IN_OBS_SPACE']:
                    traj_batch_obs = jnp.concat((traj_batch_obs, traj_batch_obs_only_cri), axis=-1)
                    traj_batch_obs_only_cri = None

                gae, targets = jax.tree.map(lambda x: x.swapaxes(0, 1), (gae, targets))

                # RERUN NETWORK
                pi, value = network(traj_batch_obs, traj_batch_obs_only_cri)
...
def update_rec_network(runner_state, traj_batch, advantages, targets, config):
    def _update_epoch(update_state: UpdateState):
        def _update_minbatch(net_and_optim, traj_batch, advantages, targets):
            ...
            # grads_rec = jax.tree.map(lambda x: jnp.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0), grads_rec)
