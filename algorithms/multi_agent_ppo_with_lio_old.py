...
def make_train(config, env:RECEnv):
    ...
    schedule_policy = schedule_builder(config['LR_POLICY'], config['LR_POLICY_MIN'], config['FRACTION_DYNAMIC_LR_POLICY'],
                                       config['NUM_MINIBATCHES'], warm_up=config.get('WARMUP_SCHEDULE_POLICY', 0))

    schedule_incentives = schedule_builder(config['LR_INCENTIVES'], config['LR_INCENTIVES_MIN'], config['FRACTION_DYNAMIC_LR_INCENTIVES'],
                                           config['NUM_MINIBATCHES'], warm_up=config.get('WARMUP_SCHEDULE_INCENTIVES', 0))

    if config['USE_WEIGHT_DECAY']:
        tx_pol = optax.chain(
            optax.clip_by_global_norm(config['MAX_GRAD_NORM']),
            optax.adamw(learning_rate=schedule_policy, eps=1e-5),
        )
        tx_inc = optax.adamw(learning_rate=schedule_incentives, eps=1e-5)
    else:
        tx_pol = optax.chain(
            optax.clip_by_global_norm(config['MAX_GRAD_NORM']),
            optax.adam(learning_rate=schedule_policy, eps=1e-5),
        )
...
def train_wrapper(env:RECEnv, config, networks_policy, optimizers_policy, networks_incentives, optimizers_incentives,
    ...
    def train(env: RECEnv, config, network_policy, optimizer_policy, network_incentives, optimizer_incentives, rng,
        ...
        network_policy.eval()
        network_incentives.train()

        runner_state = RunnerState(networks_policy=network_policy,
                                   networks_incentives=networks_incentives,
...
def collect_trajectories(runner_state: RunnerState, config, env):
    ...
    def _env_step(runner_state: RunnerState):
        ...
        obsv, env_state, reward_env, done, info = jax.lax.stop_gradient(env.step(rng_step, env_state, actions_dict))
...
def _calculate_gae(traj_batch:Transition, last_val, config):
    ...
    tot_rewards = traj_batch.tot_rewards[..., :config['NUM_RL_AGENTS']]
    ...
    if config['NUM_RL_AGENTS'] > 0:
        ...
        targets = advantages + traj_batch.values
...
def update_incentive_network(runner_state:RunnerState, env, config):
    ...
    def inc_loss(net_inc, runner_state:RunnerState):
        ...
        def loss_second_term(traj_batch_first:Transition):
            gammas = config['GAMMA'] ** jnp.arange(config['NUM_STEPS'])
            norms = jnp.abs(traj_batch_first.int_rewards_mat).sum(axis=-1)
            
            loss = gammas[:, None, None] * norms

            print('losssss', loss.shape)
            loss = loss.mean(axis=1)
        ...
        advantages, targets = _calculate_gae(traj_batch_first, last_val_batteries, config)
        ...
        loss_second = loss_second_term(traj_batch_first)
        
        loss = loss_first + config['ALPHA'] * loss_second
        loss = loss.sum()
        return loss, runner_state

    opt = runner_state.optimizers_incentives
    ...
    runner_state = runner_state._replace(networks_incentives=networks_incentives, optimizers_incentives=opt)

    # jax.debug.print('norm grad inc {x}', x=nnx.vmap(optax.global_norm)(grads), ordered=True)
...
def test_networks(env:RECEnv, train_state:TrainState, num_iter, config, rng, rec_rule_based_policy, curr_iter=0, print_data=False):
    ...
    def _env_step(runner_state, unused):
        ...
        if config['NUM_RL_AGENTS'] > 0:
            ...
            if config['NETWORK_TYPE_BATTERIES'] == 'recurrent_actor_critic':
                pi, value, act_state_batteries, cri_state_batteries = networks_policy(obsv_rl, act_state_batteries, cri_state_batteries)
            else:
        ...
        if config['NUM_BATTERY_FIRST_AGENTS'] > 0:
            ...
            actions_battery_first = (generation - demand) / env_state.battery_states.electrical_state.v[idx_start_bf:idx_end_bf]
