...
generates the parameters dict for `RECEnv`.
"""
...
WORLD_DEFAULT = 'ernestogym/envs/multi_agent/world_default.yaml'
...
def _get_world(num_battery_houses, demand, generation, ambient_temp, market, additional_battery_obs=None, additional_rec_obs=None, **kwargs):
    ...
    with open(WORLD_DEFAULT, 'r') as f:
    ...
    if additional_battery_obs is not None:
        world['battery_observations'] += additional_battery_obs
    if additional_rec_obs is not None:
        world['rec_observations'] += additional_rec_obs
...
def get_world_metadata_from_template(template_name:str, world_kwargs=None, template_folder_path:str='ernestogym/envs/multi_agent/templates'):
