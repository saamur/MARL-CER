...
from ernestogym.ernesto.demand import Demand, DemandData
from ernestogym.ernesto.generation import Generation
from ernestogym.ernesto.market import BuyingPrice, SellingPrice
from ernestogym.ernesto.ambient_temperature import AmbientTemperature
...
class MicroGridEnv(environment.Environment[EnvState, EnvParams]):
    ...
    def step_env(self, key: chex.PRNGKey, state: EnvState, action: Union[int, float, chex.Array], params: EnvParams) -> Tuple[chex.Array, TEnvState, jnp.ndarray, jnp.ndarray, Dict[Any, Any]]:
        ...
        info = {'soc': new_battery_state.soc_state.soc,
                ...
                'r_tot': r_tot,
                'generation':Generation.get_generation(self.generation_data, new_timeframe),
                'demand': Demand.get_demand(state.demand_data, new_timeframe),
                'buy_price': BuyingPrice.get_buying_price(self.buying_price_data, new_timeframe),
                'sell_price': SellingPrice.get_selling_price(self.selling_price_data, new_timeframe),
                'p': new_battery_state.electrical_state.p,
