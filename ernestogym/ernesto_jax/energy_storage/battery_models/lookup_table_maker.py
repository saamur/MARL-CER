...
params_csv_folder = 'ernestogym/ernesto_jax/data/battery_new/params/'

def build_grid_for_lookup(data):
    if isinstance(data, str):
        data = pd.read_csv(params_csv_folder + data)
        data = data.to_numpy()
    ...
    if data.shape[1] == 2:
        ...
        array[np.rint(data[:, 0] / x_step).astype(int)] = data[:, 1]
        ...
        matrix[np.rint(data[:, 0] / x_step).astype(int), np.rint(data[:, 1] / y_step).astype(int)] = data[:, 2]

        return matrix, x_step, y_step

    else:
        raise NotImplementedError('Only single or two input functions interpolation is supported')

def get_interpolation_2d(lookup_table, x_step, y_step, x, y):
...
def get_interpolation_1d(lookup_table, x_step, x):
    point = jnp.array([x])
    coord = point / x_step
    coord = coord[:, None]
